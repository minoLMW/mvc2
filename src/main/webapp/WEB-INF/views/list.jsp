...
<head>
    ...
    <style>
        * {margin:0; padding:0; font-family:'SF_HambakSnow';}
        ...
        .write-btn a {display:inline-block; height:50px; line-height:50px; padding:0 16px; font-size:20px; border-radius:12px; border:none; color:#fff; background-color:#cdb4db; text-decoration:none;}
        /* 눈 내리는 배경 클래스 */
        .snowing {
            background-image:
...
<div class="container">
    <div class="mode-change">
        <button type="button" class="gallerymodebtn">
            갤러리 모드 전환
        </button>
        <button type="button" class="listmodebtn">
            리스트 모드 전환
        </button>
    </div>
    <div class="write-btn">
...
<script src="https://kit.fontawesome.com/db42754248.js" crossorigin="anonymous"></script>
<script>
    // 갤러리 모드 켜기
    document.querySelector('.gallerymodebtn').addEventListener('click', function () {
        const table = document.querySelector('table');
        table?.classList.add('gallery-mode');
    });

    // 리스트 모드로 되돌리기 (갤러리 모드 제거)
    document.querySelector('.listmodebtn').addEventListener('click', function () {
        const table = document.querySelector('table');
        table?.classList.remove('gallery-mode');
    });
</script>
</body>
</html>
